...
```php
$nonce = wp_create_nonce( 'csbxwoo_action' );
```

### Verify Nonce (Form)

```php
if ( ! isset( $_POST['csbxwoo_nonce'] ) ) {
	wp_die( esc_html__( 'Nonce missing', 'codesoup-woo-boxnow' ) );
}

if ( ! wp_verify_nonce( $_POST['csbxwoo_nonce'], 'csbxwoo_action' ) ) {
	wp_die( esc_html__( 'Invalid nonce', 'codesoup-woo-boxnow' ) );
}
```
...
```php
if ( ! check_ajax_referer( 'csbxwoo_ajax_nonce', 'nonce', false ) ) {
	wp_send_json_error(
		array(
...
```php
<form method="post">
	<?php wp_nonce_field( 'csbxwoo_action', 'csbxwoo_nonce' ); ?>
...
```php
$url = wp_nonce_url(
	admin_url( 'admin.php?page=csbxwoo&action=delete&id=123' ),
	'csbxwoo_delete'
);
```
...
```php
if ( ! isset( $_GET['_wpnonce'] ) || ! wp_verify_nonce( $_GET['_wpnonce'], 'csbxwoo_delete' ) ) {
	wp_die( esc_html__( 'Invalid nonce', 'codesoup-woo-boxnow' ) );
}
```
...
if ( ! current_user_can( 'manage_options' ) ) {
	wp_die( esc_html__( 'Permission denied', 'codesoup-woo-boxnow' ) );
}
```
...
```php
if ( ! current_user_can( 'csbxwoo_custom_capability' ) ) {
	wp_die( esc_html__( 'Permission denied', 'codesoup-woo-boxnow' ) );
}
```
...
class SecureForm {
	...
	public function render(): void {
		...
		<form method="post" action="<?php echo esc_url( admin_url( 'admin-post.php' ) ); ?>">
			<?php wp_nonce_field( 'csbxwoo_form', 'csbxwoo_nonce' ); ?>
			<input type="hidden" name="action" value="csbxwoo_submit" />

			<label><?php esc_html_e( 'Name:', 'codesoup-woo-boxnow' ); ?></label>
			<input type="text" name="name" value="<?php echo esc_attr( $name ); ?>" />

			<label><?php esc_html_e( 'Email:', 'codesoup-woo-boxnow' ); ?></label>
			<input type="email" name="email" value="<?php echo esc_attr( $email ); ?>" />

			<button type="submit">
				<?php esc_html_e( 'Submit', 'codesoup-woo-boxnow' ); ?>
			</button>
		</form>
		<?php
	}
	...
	public function handle_submit(): void {
		// Check nonce
		if ( ! isset( $_POST['csbxwoo_nonce'] ) || ! wp_verify_nonce( $_POST['csbxwoo_nonce'], 'csbxwoo_form' ) ) {
			wp_die( esc_html__( 'Invalid nonce', 'codesoup-woo-boxnow' ) );
		}
		...
		if ( empty( $name ) || empty( $email ) ) {
			wp_die( esc_html__( 'Required fields missing', 'codesoup-woo-boxnow' ) );
		}

		// Process
		update_option( 'csbxwoo_name', $name );
		update_option( 'csbxwoo_email', $email );
		...
		wp_safe_redirect(
			add_query_arg(
				'message',
				'success',
				admin_url( 'admin.php?page=csbxwoo' )
			)
		);
		exit;
	}
