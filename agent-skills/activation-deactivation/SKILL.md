...
class Activator {
	...
	public static function activate(): void {
		...
		// Set activation flag
		update_option( 'csbxwoo_activated', true );
		update_option( 'csbxwoo_version', '1.0.0' );
	}
	...
	private static function create_tables(): void {
		...
		$table_name      = $wpdb->prefix . 'csbxwoo_data';
	...
	private static function set_default_options(): void {
		$defaults = array(
			'csbxwoo_setting_1' => 'default_value',
			'csbxwoo_setting_2' => true,
			'csbxwoo_setting_3' => array( 'option1', 'option2' ),
		);
...
class Deactivator {
	...
	public static function deactivate(): void {
		...
		update_option( 'csbxwoo_deactivated', true );
	}

	private static function clear_scheduled_events(): void {
		$timestamp = wp_next_scheduled( 'csbxwoo_daily_task' );
		if ( $timestamp ) {
			wp_unschedule_event( $timestamp, 'csbxwoo_daily_task' );
		}
	}

	private static function clear_transients(): void {
		delete_transient( 'csbxwoo_cache_key' );
		delete_transient( 'csbxwoo_api_data' );
	}
}
```
...
// Delete options
delete_option( 'csbxwoo_setting_1' );
delete_option( 'csbxwoo_setting_2' );
delete_option( 'csbxwoo_setting_3' );
delete_option( 'csbxwoo_activated' );
delete_option( 'csbxwoo_version' );

// Delete transients
delete_transient( 'csbxwoo_cache_key' );

// Delete user meta
delete_metadata( 'user', 0, 'csbxwoo_user_setting', '', true );

// Drop custom tables
global $wpdb;
$wpdb->query( "DROP TABLE IF EXISTS {$wpdb->prefix}csbxwoo_data" );

// Clear scheduled events
wp_clear_scheduled_hook( 'csbxwoo_daily_task' );
```
...
register_activation_hook( __FILE__, array( \CodeSoup\BoxNow\Core\Activator::class, 'activate' ) );
register_deactivation_hook( __FILE__, array( \CodeSoup\BoxNow\Core\Deactivator::class, 'deactivate' ) );
```
...
private static function maybe_upgrade(): void {
	$current_version = get_option( 'csbxwoo_version', '0.0.0' );
	$new_version     = '1.1.0';

	if ( version_compare( $current_version, $new_version, '<' ) ) {
		self::upgrade_to_1_1_0();
		update_option( 'csbxwoo_version', $new_version );
	}
}
...
private static function schedule_events(): void {
	if ( ! wp_next_scheduled( 'csbxwoo_daily_task' ) ) {
		wp_schedule_event(
			time(),
			'daily',
			'csbxwoo_daily_task'
...
public static function activate(): void {
	// Set redirect flag
	set_transient( 'csbxwoo_activation_redirect', true, 30 );
}
```
...
public function maybe_redirect(): void {
	if ( get_transient( 'csbxwoo_activation_redirect' ) ) {
		delete_transient( 'csbxwoo_activation_redirect' );

		wp_safe_redirect(
			admin_url( 'admin.php?page=csbxwoo-welcome' )
		);
		exit;
	}
}
